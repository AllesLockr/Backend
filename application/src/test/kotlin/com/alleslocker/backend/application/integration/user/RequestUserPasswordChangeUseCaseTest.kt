...
class RequestUserPasswordChangeUseCaseTest :
    FreeSpec({
        ...
        val useCase: RequestUserPasswordChangeUseCase = ctx.useCaseFactory.make(RequestUserPasswordChangeUseCase::class)
        ...
        "should reject if user already requested to change password" {
            ctx.userGateway.save(
                ctx.userGateway.findById(UserId(targetUserId))!!.copy(mustChangePassword = true),
            )
