...
`ktlintCheck`/`ktlintFormat` need `--sun-misc-unsafe-memory-access=allow` JVM arg (set in `build.gradle` for Java 24+).
...
- **lint.yml**: `./gradlew ktlintCheck` on JDK 24 (Temurin), runs on every push/PR
- **deploy-demo.yml**: manual `workflow_dispatch`, builds Docker image, deploys via docker-compose on self-hosted runner

## Style & conventions

- `.editorconfig`: 4-space indent, LF, 120 max line width, star imports disabled (IntelliJ threshold = `Int.MAX_VALUE`)
